        ...
        {open && (
          ...
          >
            ...
            <div onClick={() => isMobile && setOpen(false)}>
              <Badge href="/resume" text="Read My Resume" />
            </div>
          </motion.div>
        )}
      </AnimatePresence>
      <button
        className="fixed lg:hidden top-4 left-4 h-8 w-8 border border-border rounded-full backdrop-blur-xs flex items-center justify-center z-50"
      ...
      {navlinks.map((link: Navlink) => (
        <Link
          ...
          className={twMerge(
            ...
            isActive(link.href) && "bg-background shadow-lg text-foreground"
          )}
        >
          <link.icon
            className={twMerge(
...
const SidebarHeader = () => {
  return (
    <div className="flex space-x-2">
      <Image
        src="/images/my_profile_picture_blurred.jpg"
        alt="Avatar"
        height="40"
        width="40"
      ...
      <div className="flex text-sm flex-col">
        <p className="font-bold text-primary">Mayowa Obisesan</p>
