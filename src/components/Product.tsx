        ...
        {product.images.map((image, idx) => (
          ...
          >
            <Image
              ...
              height="1000"
              width="1000"
              className="h-14 w-16 md:h-40 md:w-60 object-cover object-top mr-4 mb-r border rounded-lg border-border"
            />
          </button>
        ))}
      </div>
      <div className="flex lg:flex-row justify-between items-center flex-col mt-20">
        ...
        <div className="flex flex-wrap space-x-2 space-y-2 md:mb-1 mt-2 md:mt-0">
          {product.stack?.map((stack: string) => (
            <span
              key={stack}
              className="text-xs md:text-xs lg:text-xs bg-muted-foreground px-2 py-1 rounded-xs text-muted"
            >
              {stack}
            </span>
          ))}
